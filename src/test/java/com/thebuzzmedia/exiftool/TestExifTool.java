...
public class TestExifTool {
  ...
  private void testFile(ExifToolService tool, String resource, int size, int width, int height, String model, int iso, String shutterRaw,
      double shutter) throws URISyntaxException, IOException {
    File imageFile = new File(getClass().getResource(resource).toURI());
    Map<MetadataTag, String> metadata = tool.getImageMeta4d(imageFile, new ReadOptions().withNumericOutput(Format.HUMAN_READABLE),
        Tag.values());
  ...
  public void testWriteTagStringNonDaemon() throws Exception {
    try (ExifToolService tool = create()) {
      ...
      metadata = tool.getImageMeta4d(imageFile.toFile(), new ReadOptions().withNumericOutput(Format.HUMAN_READABLE), tags1);
  ...
  public void testWriteTagStringInvalidformat() throws Exception {
    try (ExifToolService tool = create(Feature.STAY_OPEN)) {
      Path imageFile = Paths.get("target", "nexus-s-electric-cars-tochange.jpg");
      Files.copy(Paths.get(getClass().getResource("/nexus-s-electric-cars.jpg").toURI()), imageFile, StandardCopyOption.REPLACE_EXISTING);
      ...
      metadata = tool.getImageMeta4d(imageFile.toFile(), new ReadOptions().withNumericOutput(Format.HUMAN_READABLE),
  ...
  public void testWriteTagNumberNonDaemon() throws Exception {
    try (ExifToolService tool = create()) {
      ...
      Map<MetadataTag, String> metadata = tool.getImageMeta4d(imageFile.toFile(),
          new ReadOptions().withNumericOutput(Format.HUMAN_READABLE), tags);
      assertEquals("Orientation tag starting value is wrong", "Horizontal (normal)", metadata.get(Tag.ORIENTATION));
  ...
  public void testWriteMulipleTag() throws Exception {
    try (ExifToolService tool = create(Feature.STAY_OPEN)) {
      ...
      // Test what orientation value is at the start
      Map<MetadataTag, String> metadata = tool.getImageMeta4d(imageFile.toFile(),
          new ReadOptions().withNumericOutput(Format.HUMAN_READABLE), Tag.ORIENTATION, Tag.DATE_TIME_ORIGINAL);
      assertEquals("Orientation tag starting value is wrong", "Horizontal (normal)", Tag.ORIENTATION.getRawValue(metadata));
      ...
      // Finally check the updated value
      metadata = tool.getImageMeta4d(imageFile.toFile(), new ReadOptions().withNumericOutput(Format.HUMAN_READABLE), Tag.ORIENTATION,
          Tag.DATE_TIME_ORIGINAL);
  ...
  public void testWritingWithImplicitTypes() throws Exception {
    try (ExifToolService tool = create(new ReadOptions().withNumericOutput(true).withConvertTypes(true), Feature.MWG_MODULE)) {
      ...
      assertEquals("Wrong starting value", formatter.parse("2010:12:10 17:07:05"), MwgTag.DATE_TIME_ORIGINAL.getValue(metadata));
  ...
  public void testReadingUtf8NamesWithStayOpen() throws Exception {
    try (ExifToolService tool = create(new ReadOptions().withNumericOutput(true).withConvertTypes(true), Feature.STAY_OPEN)) {
