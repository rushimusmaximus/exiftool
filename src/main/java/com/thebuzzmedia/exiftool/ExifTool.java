 ...
 * <p/>
 * There are a number of other basic Java wrappers to ExifTool available online, but most of them only abstract out the
 * actual Java-external-process execution logic and do no additional work to make integration with the external ExifTool
 * any easier or intuitive from the perspective of the Java application written to make use of ExifTool.
 * <p/>
 * This class was written in order to make integration with ExifTool inside of a Java application seamless and
 * performant with the goal being that the developer can treat ExifTool as if it were written in Java, garnering all of
 * the benefits with none of the added headache of managing an external native process from Java.
 * <p/>
 * Phil Harvey's ExifTool is written in Perl and runs on all major platforms (including Windows) so no portability
 * issues are introduced into your application by utilizing this class.
 * <h3>Usage</h3>
 * Assuming ExifTool is installed on the host system correctly and either in the system path or pointed to by
 * {@link #exifCmd}, using this class to communicate with ExifTool is as simple as creating an instance (
 * <code>ExifTool tool = new ExifTool()</code>) and then making calls to {@link #getImageMeta(File, Tag...)} or
 * {@link #getImageMeta(File, Format, Tag...)} with a list of {@link Tag}s you want to pull values for from the given
 * image.
 * <p/>
 * In this default mode, calls to <code>getImageMeta</code> will automatically start an external ExifTool process to
 * handle the request. After ExifTool has parsed the tag values from the file, the external process exits and this class
 * parses the result before returning it to the caller.
 * <p/>
 ...
 * <p/>
 * While each {@link Tag} provides a hint at which format the resulting value for that tag is returned as from ExifTool
 * (see {@link Tag#getType()}), that only applies to values returned with an output format of {@link Format#NUMERIC} and
 * it is ultimately up to the caller to decide how best to parse or convert the returned values.
 * <p/>
 ...
 * added a new persistent-process feature that allows ExifTool to stay running in a daemon mode and continue accepting
 * commands via a file or stdin.
 * <p/>
 * This new mode is controlled via the <code>-stay_open True/False</code> command line argument and in a busy system
 * that is making thousands of calls to ExifTool, can offer speed improvements of up to <strong>60x</strong> (yes,
 * really that much).
 * <p/>
 * This feature was added to ExifTool shortly after user <a href="http://www.christian-etter.de/?p=458">Christian Etter
 * discovered</a> the overhead for starting up a new Perl interpreter each time ExifTool is loaded accounts for roughly
 * <a href= "http://u88.n24.queensu.ca/exiftool/forum/index.php/topic,1402.msg6121.html#msg6121" >98.4% of the total
 * runtime</a>.
 * <p/>
 * Support for using ExifTool in daemon mode is enabled by passing {@link Feature#STAY_OPEN} to the constructor of the
 * class when creating an instance of this class and then simply using the class as you normally would. This class will
 * manage a single ExifTool process running in daemon mode in the background to service all future calls to the class.
 * <p/>
 * Because this feature requires ExifTool 8.36 or later, this class will actually verify support for the feature in the
 * version of ExifTool pointed at by {@link #exifCmd} before successfully instantiating the class and will notify you
 * via an {@link UnsupportedFeatureException} if the native ExifTool doesn't support the requested feature.
 * <p/>
 * In the event of an {@link UnsupportedFeatureException}, the caller can either upgrade the native ExifTool upgrade to
 * the version required or simply avoid using that feature to work around the exception.
 ...
 * <code>0</code> or less disabled the automatic cleanup process and requires you to cleanup ExifTool instances on your
 * own by calling {@link #close()} manually.
 * <p/>
 * Any class activity by way of calls to <code>getImageMeta</code> will always reset the inactivity timer, so in a busy
 * system the cleanup thread could potentially never run, leaving the original host ExifTool process running forever
 * (which is fine).
 * <p/>
 ...
 * would want to use ExifTool in daemon mode (pass {@link Feature#STAY_OPEN} to the constructor of this class) and when
 * done, instead of {@link #close()}-ing the instance of this class and throwing it out, you can keep the reference
 ...
 * <h3>Thread Safety</h3>
 * Instances of this class are <strong>not</strong> Thread-safe. Both the instance of this class and external ExifTool
 * process maintain state specific to the current operation. Use of instances of this class need to be synchronized
 * using an external mechanism or in a highly threaded environment (e.g. web application), instances of this class can
 * be used along with {@link ThreadLocal}s to ensure Thread-safe, highly parallel use.
 * <h3>Why ExifTool?</h3>
 * <a href="http://www.sno.phy.queensu.ca/~phil/exiftool">ExifTool</a> is written in Perl and requires an external
 ...
 * ExifTool. In addition, ExifTool already runs on all major platforms (including Windows), so there was not a lack of
 * portability introduced by providing an integration for it.
 * <p/>
...
public class ExifTool
{
	 ...
	 * property.
	 * <p/>
	 * If ExifTool is on your system path and running the command "exiftool" successfully executes it, leaving this
	 * value unchanged will work fine on any platform. If the ExifTool executable is named something else or not in the
	 * system path, then this property will need to be set to point at it before using this class.
	 * <p/>
	 ...
	 * cleanup the external ExifTool process and read/write streams by calling {@link #close()}.
	 * <p/>
	...
	public ExifTool(String exifCmd, long processCleanupDelay, Feature... features)
	{
		...
		if( features != null && features.length > 0)
		{
			...
			{
				if( !feature.isSupported( exifVersion)) { throw new UnsupportedFeatureException( feature); }
	 ...
	 * ExifTool and ensure the requested feature is supported in the current local install.
	 * <p/>
	 * The external call to ExifTool to confirm feature support is only ever done once per JVM session and stored in a
	 * <code>static final</code> {@link Map} that all instances of this class share.
	 * 
	 * @param feature
	 ...
	 *         confirmed to work with the currently installed ExifTool or <code>false</code> if it is not supported.
	 * 
	 ...
	 * enabled.
	 * <p/>
	 * <strong>NOTE</strong>: Calling this method does not preclude this instance of {@link ExifTool} from being
	 ...
	 *         daemon mode associated with this class utilizing the {@link Feature#STAY_OPEN} feature, otherwise returns
	 *         <code>false</code>.
	 */
	 ...
	 * supported by the underlying ExifTool install where as this method tells the caller if the given feature has been
	 * enabled for use in this particular instance.
	 * 
	 * @param feature
	 ...
	 *         enabled on this instance of {@link ExifTool}, otherwise returns <code>false</code>.
	 * 
	 * @throws IllegalArgumentException
	 *         if <code>feature</code> is <code>null</code>.
	 */
	...
	public Map<Tag, String> getImageMeta(File image, Tag... tags) throws IllegalArgumentException, SecurityException, IOException
	{

		return getImageMeta( image, Format.NUMERIC, tags);
	}

	public Map<Tag, String> getImageMeta(File image, Format format, Tag... tags) throws IllegalArgumentException, SecurityException,
		IOException
	{
	...
	public Map<String, String> getImageMeta(File image, Format format, TagGroup... tags) throws IllegalArgumentException,
		...
		throws IllegalArgumentException, SecurityException, IOException
	{
		...
		if( tags == null || tags.length == 0) { throw new IllegalArgumentException(
		...
		if( !image.canRead()) { throw new SecurityException( "Unable to read the given image [" + image.getAbsolutePath()
	...
	private Map<String, String> processStayOpen(List<String> args) throws IOException
	{
		...
		if( shuttingDown.get()) { throw new IOException( "Shutting Down"); }
	...
	public static class ExifProcess
	{
		...
		public static ExifProcess _execute(boolean keepAlive, List<String> args)
		{
			...
			{
				String message = "Unable to start external ExifTool process using the execution arguments: " + args
					+ ". Ensure ExifTool is installed correctly and runs using the command path '" + args.get( 0)
	 ...
	 * <p/>
	 * These flags are different from {@link Tag}s in that a "feature" is determined to be a special functionality of
	 * the underlying ExifTool executable that requires a different code-path in this class to take advantage of; for
	 * example, <code>-stay_open True</code> support.
	 * 
	 * @author Riyad Kalla (software@thebuzzmedia.com)
	 * @since 1.1
	 */
	...
	// ================================================================================
	/**
	 * Enum used to define the 2 different output formats that {@link Tag} values can be returned in: numeric or
	 * human-readable text.
	 * <p/>
	 * ExifTool, via the <code>-n</code> command line arg, is capable of returning most values in their raw numeric form
	 * (e.g. Aperture="2.8010323841") as well as a more human-readable/friendly format (e.g. Aperture="2.8").
	 * <p/>
	 * While the {@link Tag}s defined on this class do provide a hint at the type of the result (see
	 * {@link Tag#getType()}), that hint only applies when the {@link Format#NUMERIC} form of the value is returned.
	 * <p/>
	 * If the caller finds the human-readable format easier to process, {@link Format#HUMAN_READABLE} can be specified
	 * when calling {@link ExifTool#getImageMeta(File, Format, Tag...)} and the returned {@link String} values processed
	 * manually by the caller.
	 * <p/>
	 * In order to see the types of values that are returned when {@link Format#HUMAN_READABLE} is used, you can check
	 * the comprehensive <a href="http://www.sno.phy.queensu.ca/~phil/exiftool/TagNames/index.html"> ExifTool Tag
	 * Guide</a>.
	 * <p/>
	 * This makes sense with some values like Aperture that in {@link Format#NUMERIC} format end up returning as
	 * 14-decimal-place, high precision values that are near the intended value (e.g. "2.79999992203711" instead of just
	 * returning "2.8"). On the other hand, other values (like Orientation) are easier to parse when their numeric value
	 * (1-8) is returned instead of a much longer friendly name (e.g. "Mirror horizontal and rotate 270 CW").
	 * 
	 * @author Riyad Kalla (software@thebuzzmedia.com)
	 * @since 1.1
	 */
	 ...
	 * <p/>
	 * All replies from ExifTool are parsed as {@link String}s and using the type hint from each {@link Tag} can easily
	 ...
	 * prefer tag values returned in {@link Format#HUMAN_READABLE} format and to avoid any compatibility issues with
	 * future versions of ExifTool if a tag's return value is changed. This approach to leaving returned tag values as
	 * strings until the caller decides they want to parse them is a safer and more robust approach.
	 * <p/>
	 * The types provided by each tag are merely a hint based on the <a
	 * href="http://www.sno.phy.queensu.ca/~phil/exiftool/TagNames/index.html" >ExifTool Tag Guide</a> by Phil Harvey;
	 * the caller is free to parse or process the returned {@link String} values any way they wish.
	 * <h3>Tag Support</h3>
	 * ExifTool is capable of parsing almost every tag known to man (1000+), but this class makes an attempt at
	 ...
	 * (iPhone, Android) as well as cameras like simple point and shoots as well as DSLRs. As an additional source of
	 * input the list of supported/common EXIF formats that Flickr supports was also reviewed to ensure the most
	 * common/useful tags were being covered here.
	 * <p/>
	 * Please email me or <a href="https://github.com/thebuzzmedia/imgscalr/issues">file an issue</a> if you think this
	 * list is missing a commonly used tag that should be added to it.
	 * 
	 * @author Riyad Kalla (software@thebuzzmedia.com)
	 * @since 1.1
	 */
	public enum Tag
	{
		 ...
		 *         numeric types
		 *         in Java via calls to the respective <code>parseXXX</code> methods defined on all the numeric wrapper
		 *         classes (e.g. {@link Integer#parseInt(String)} , {@link Double#parseDouble(String)} and so on).
		 ...
		 *         performs an implicit/unchecked cast to the type <code>T</code> before returning the parsed result of
		 *         the
		...
		public <T> T parseValue(String value) throws IllegalArgumentException
		{
			...
			if( value != null)
			{
				...
				else if( String.class.isAssignableFrom( this.getType())) { return (T)value; }
			}
			return null;
		}
