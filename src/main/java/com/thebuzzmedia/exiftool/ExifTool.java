...
public class ExifTool {
	...
	// ================================================================================
	/**
	 * Represents an external exif process. Works for both single use and keep
	 * alive modes. This is the actual process, with streams for reading and
	 * writing data.
	 */
	public static final class ExifProcess {
		...
		public ExifProcess(boolean keepAlive, List<String> args) {
			...
			log.debug(String
					.format("Attempting to start ExifTool process using args: %s",
							args));
			try {
		...
		public synchronized Map<String, String> readResponse()
			...
			while ((line = reader.readLine()) != null) {
				...
				if (pair.length == 2) {
					...
					log.debug(String.format("\tRead Tag [name=%s, value=%s]",
							pair[0], pair[1]));
				}
	...
	/**
	 * A Proxy to an Exif Process, will restart if backing exif process died, or
	 * run new one on every call.
	 * 
	...
	public interface ExifProxy {
		...
		public Map<String, String> execute(long runTimeoutMills,
	...
	public static class KeepAliveExifProxy implements ExifProxy {
		...
		public KeepAliveExifProxy(String exifCmd, List<String> baseArgs) {
			inactivityTimeout = Long.getLong(ENV_EXIF_TOOL_PROCESSCLEANUPDELAY,
					DEFAULT_PROCESS_CLEANUP_DELAY);
			...
			while (attempts < 3 && !shuttingDown.get()) {
				...
				try {
					...
					if (STREAM_CLOSED_MESSAGE.equals(ex.getMessage())
						...
						// process has died)
						log.warn(String
								.format("Caught IOException(\"%s\"), will restart daemon",
	...
	public static class ReadOptions {
		...
		private ReadOptions(long runTimeoutMills, boolean convertTypes,
				boolean numericOutput, boolean showDuplicates,
				boolean showEmptyTags) {
		 ...
		 * exiftool. If this is enabled then {@link MetadataTag#getType()} is
		 * used to cast the string into a java type.
		 */
		 ...
		 * Aperture="2.8010323841") as well as a more human-readable/friendly
		 * format (e.g. Aperture="2.8").
		 * <p/>
		 * While the {@link Tag}s defined on this class do provide a hint at the
		 * type of the result (see {@link MetadataTag#getType()}), that hint
		 * only applies when the numeric value is returned.
		 * <p/>
		 * If the caller finds the human-readable format easier to process, Set
		 * this to false, the default.
		 * <p/>
		 * In order to see the types of values that are returned when human
		 * readable is used (default), you can check the comprehensive <a href=
		 * "http://www.sno.phy.queensu.ca/~phil/exiftool/TagNames/index.html">
		 * ExifTool Tag Guide</a>.
		 * <p/>
		 * This makes sense with some values like Aperture that in numeric
		 * format end up returning as 14-decimal-place, high precision values
		 * that are near the intended value (e.g. "2.79999992203711" instead of
		 * just returning "2.8"). On the other hand, other values (like
		 * Orientation) are easier to parse when their numeric value (1-8) is
		 * returned instead of a much longer friendly name (e.g.
		...
		public ReadOptions withShowDuplicates(boolean enabled) {
			return new ReadOptions(runTimeoutMills, convertTypes,
					numericOutput, enabled, showEmptyTags);
		}

		public ReadOptions withShowEmptyTags(boolean enabled) {
			return new ReadOptions(runTimeoutMills, convertTypes,
					numericOutput, showDuplicates, enabled);
		}
	}
	...
	public static class WriteOptions {
		...
		/**
		 * ExifTool automatically makes a backup copy a file before writing
		 * metadata tags in the form "file.ext_original", by default this tool
		 * will delete that original file after the writing is done.
		 */
	...
	public enum Feature {
		 ...
		 * reconciled when reading, and synchronized when writing. The MWG
		 * Composite tags below are designed to aid in the implementation of
		 * these recommendations. Will add the args " -use MWG"
		 * 
		 * @see <a
	...
	// ================================================================================
	/**
	 * Base type for all "tag" passed to exiftool. The key is the value passed
	 * to the exiftool like "-creator". The Types is used for automatic type
	 * conversions.
	 * 
	 */
