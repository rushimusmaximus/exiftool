...
/**
 * A Thread which reads lines from a BufferedReader and puts them in a queue, so they can be read from with out blocking.
 * This is used when reading from a process.err input.
 *
...
public class LineReaderThread extends Thread {
  ...
  private BlockingQueue<String> lineBuffer = new ArrayBlockingQueue<String>(10,true);

  public LineReaderThread(String name, BufferedReader reader) {
    super(name);
    this.reader = reader;
  }
  public LineReaderThread(ThreadGroup group, String name, BufferedReader reader) {
    super(group, name);
    this.reader = reader;
  }
